#!/usr/bin/env python3
"""Verify that ws://localhost/ws/id reaches liyanlabs-rendezvous via nginx."""
import os, socket, struct, sys, base64

HOST = os.environ.get("LIYANLABS_HOST", "127.0.0.1")
PORT = int(os.environ.get("LIYANLABS_PORT", "80"))
PATH = os.environ.get("LIYANLABS_PATH", "/ws/id")
...
if body[:2] == b"\x3a\x00":
    print("OK: nginx → liyanlabs-rendezvous WebSocket path works", file=sys.stderr)
    sys.exit(0)
else:
